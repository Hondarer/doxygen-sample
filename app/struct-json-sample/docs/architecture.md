...
`structgen` (ヘッダー解析ツール) は、ソース テキストから「型のスペリング」「フィールド名」「配列次元の整数リテラル」だけを抽出します。フィールドのオフセットやサイズ (`offsetof`/`sizeof`) は自分で計算せず、生成する C コードに `offsetof(Type, field)` のような式をそのまま埋め込み、**実ヘッダーを `#include` した状態でコンパイラに計算させます**。
