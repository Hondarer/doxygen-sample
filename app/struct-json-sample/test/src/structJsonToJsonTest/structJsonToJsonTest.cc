...
const sj_field_desc k_address_fields[] = {
    {"city", SJ_FIELD_CHAR_ARRAY, 0, offsetof(address, city), sizeof(char), 1, sizeof(((address *)0)->city),
     nullptr},
    {"zip", SJ_FIELD_INT, 0, offsetof(address, zip), sizeof(int), 1, 0, nullptr},
};
const sj_struct_desc k_address_desc = {"address", sizeof(address), k_address_fields, 2};

const sj_field_desc k_sample_fields[] = {
    {"id", SJ_FIELD_INT, 0, offsetof(sample_data, id), sizeof(int), 1, 0, nullptr},
    {"flags", SJ_FIELD_UNSIGNED, 0, offsetof(sample_data, flags), sizeof(unsigned int), 1, 0, nullptr},
    {"ratio", SJ_FIELD_DOUBLE, 0, offsetof(sample_data, ratio), sizeof(double), 1, 0, nullptr},
    {"temp", SJ_FIELD_FLOAT, 0, offsetof(sample_data, temp), sizeof(float), 1, 0, nullptr},
    {"name", SJ_FIELD_CHAR_ARRAY, 0, offsetof(sample_data, name), sizeof(char), 1, sizeof(((sample_data *)0)->name), nullptr},
    {"home", SJ_FIELD_STRUCT, 0, offsetof(sample_data, home), sizeof(address), 1, 0, &k_address_desc},
    {"others", SJ_FIELD_STRUCT, 0, offsetof(sample_data, others), sizeof(address), 2, 0, &k_address_desc},
    {"scores", SJ_FIELD_INT, 0, offsetof(sample_data, scores), sizeof(int), 3, 0, nullptr},
};
const sj_struct_desc k_sample_desc = {"sample_data", sizeof(sample_data), k_sample_fields, 8};
} // namespace
...
TEST_F(structJsonToJsonTest, test_scalar_nested_and_array_fields)
{
    ...
    EXPECT_STREQ(
        "abc",
        cJSON_GetStringValue(cJSON_GetObjectItemCaseSensitive(json, "name"))); // [確認_正常系] - char[] が文字列になること。
