...
const sj_struct_desc k_sample_desc = {"sample_data", sizeof(sample_data), k_sample_fields, 4};
} // namespace
...
TEST_F(structJsonFromJsonTest, test_char_array_too_small)
{
    ...
    // Act
    int actual_ret = sj_from_json(&k_sample_desc, json, &s); // [手順_異常系] - バッファーを超える文字列を渡して呼び出す。
