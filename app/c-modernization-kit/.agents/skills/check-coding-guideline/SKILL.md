...
8. **標準時刻型 (com_util_timespec)**
    - `struct timespec` を直接使用せず `com_util_timespec` を使う。native 変換は to_native / from_native に集約する
    - → 原典「標準時刻型 (com_util_timespec)」を参照

9. **Doxygen コード例内のコメント形式**
...
2. 上記 9 項目について、原典の該当節を読みながら判定する
...
- 時刻の受け渡しに `struct timespec` を直接使用していないか
