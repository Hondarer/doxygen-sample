...
testfw にある `Mock_zlib` は、呼び出し元情報を受ける初期化関数 2 個のモックです。  
この app のモックとは API と ABI が異なるため、同じテスト実行ファイル内では混在させません。  
makefw の app 依存解決により、この app の `test/include` と `test/lib` を testfw より先に探索します。  
testfw の `include_override/zlib.h` 経由で読み込まれた場合は、実 API 宣言をそのまま使用し、呼び出し元情報を追加する旧モックのマクロを定義しません。  
既存の testfw モックを使う app は、モック移行なしに zlib への依存を追加しないでください。
