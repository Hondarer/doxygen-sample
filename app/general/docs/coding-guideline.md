...
ローカル変数を宣言時に全体ゼロ クリアする場合は、集成体初期化子 `= {0}` を使用し、`memset(&v, 0, sizeof(v))` は使用しません。  
対象は構造体に限らず、共用体、配列も含みます。

宣言後に、すでに使用中の変数を途中でゼロ クリアし直す場合は、この規則の対象外とし、引き続き `memset` を使用します。  
`= {0}` は宣言と一体の初期化子であり、宣言済みの変数への再代入や、ポインター引数が指す既存オブジェクトの途中クリアには使えないためです。
...
> `memset` は実行時に呼び出す関数である一方、`= {0}` は宣言と初期化が一体であり、対象の型とサイズの対応をコンパイル時に検査できます。  
