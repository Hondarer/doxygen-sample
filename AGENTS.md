...
- マルチフェーズ テスト (1 つのテスト関数内で Arrange/Pre-Assert/Act/Assert のサイクルを複数回含むテスト。部分反復を含む) は、`framework/testfw/docs/about-test-phase.md` の「シングルフェーズ テストとマルチフェーズ テスト」に従い、実際に繰り返す構造コメントにだけ `_N` を付与すること。1 回目のサイクルには番号を付けず、`// Cleanup` にも番号を付けないこと。
